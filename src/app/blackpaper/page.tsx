    ...
    <main className="site-shell inner-site docs-site blackpaper-site">
      ...
      <article className="blackpaper-body" aria-label="Bittensor Relics Blackpaper">
        <Chapter number="00" title="Executive summary" id="summary">
          ...
          <div className="paper-state-grid"><div><span>Verified on testnet</span><strong>Atomic Buy &amp; Burn with an exact WebP image in finalized history</strong></div><div><span>Implemented</span><strong>Strict replay rules, numbering, ownership transfers, proof APIs, and listing discovery</strong></div><div><span>Not deployed</span><strong>Public indexer databases, independent replay service, and live explorer data</strong></div><div><span>Proposed</span><strong>Registered subnet, emissions, authority collections, and loss-safe paid settlement</strong></div></div>
...
remainder exact WebP image bytes`}</code></pre>
        ...
        <Chapter number="07" title="Economics of sacrifice" id="economics">
          ...
          <p>The valid claim is both narrower and more useful: every accepted mint proves real TAO demand against a selected subnet pool and the exact amount of alpha relinquished by the creator. It creates a public cultural receipt tied to that alpha economy. Communities may value those receipts, and repeated use may generate pool activity and fees, but market outcomes remain uncertain.</p>
        ...
        <Chapter number="12" title="Marketplace design" id="marketplace">
          ...
          <p>This discovery model is implemented. Paid settlement is not. The required safety property is simple to state and difficult to achieve without an EVM contract: <b>a buyer must not lose TAO unless the same enforced operation gives the buyer canonical protocol ownership.</b> Two unrelated signatures do not satisfy that property. A server-held escrow does not make it trustless. Until a native non-EVM settlement path can enforce payment and transfer together, the public interface remains read-only for purchases.</p>
          <p>The future market may support direct transfers, fixed-price orders, bids, and collection views, but no feature is worth weakening the ownership rules. Wallets sign exact payloads; relayers never control artifacts; indexes verify the seller at the current nonce; and settlement stays disabled whenever ownership, price, finality, or execution cannot be bound into one loss-safe outcome.</p>
        </Chapter>
        ...
        <Chapter number="14" title="Evidence and present status" id="evidence">
          ...
          <div className="paper-status-table"><div><span>Works today</span><ul><li>Live testnet subnet discovery and quotes</li><li>Injected SS58 wallet signing</li><li>Local image conversion and exact BRI1 encoding</li><li>Atomic Buy &amp; Burn plus on-chain inscription</li><li>Finality verification and evidence export</li><li>Strict indexer, transfers, and listing code</li><li>Independent-miner conformance simulation</li></ul></div><div><span>Not live today</span><ul><li>Public primary and replay databases</li><li>Populated Explore and My Relics views</li><li>Registered subnet, miners, validators, or emissions</li><li>Authority-defined collections</li><li>Exact alpha-target minting</li><li>Mainnet minting</li><li>Paid marketplace settlement</li><li>Native Subtensor NFT ownership</li></ul></div></div>
          <p>The founding object exists in finalized testnet history, but the public explorer remains empty until deployed workers replay from the activation block. The API reports <code>INDEXER_UNAVAILABLE</code> while its database is absent. That is the intended failure mode.</p>
        </Chapter>

        <Chapter number="15" title="Path to a controlled launch" id="roadmap">
          <ol><li>Freeze and independently review one protocol and source release candidate.</li><li>Complete deterministic builds, dependency review, runtime-compatibility tests, and adversarial transaction fixtures.</li><li>Provision separate primary and replay databases; ingest from block 7,698,720; and require byte-identical state at a shared finalized checkpoint.</li><li>Publish proof APIs and verify every Explore, wallet, media, transfer, rejection, and collection view against chain evidence.</li><li>Exercise real testnet transfers and listing invalidation while paid settlement remains disabled.</li><li>Operate archival and RPC redundancy; rehearse runtime upgrades, rollbacks, stalled finality, and corrupt-cache recovery.</li><li>Run a limited public testnet beta with monitoring, published incidents, and explicit experimental warnings.</li><li>Specify miner queries, validator challenges, commitments, scoring, anti-copying tests, bandwidth budgets, and gateway quorum behavior.</li><li>Demonstrate that independent miners add verifiable availability and correctness beyond conventional replication before registering a subnet.</li><li>Review marketplace settlement and mainnet activation as separate approval gates. Neither is implied by a successful testnet subnet.</li></ol>
