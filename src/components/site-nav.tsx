...
export function SiteNav({ status, tone = "" }: { status: string; tone?: string }) {
  ...
  return (
    <nav className={`nav site-nav ${open ? "menu-open" : ""}`} aria-label="Main navigation">
      ...
      <div className="nav-desktop">
        ...
        <span className={`chain-status ${tone}`.trim()}><i aria-hidden="true" />{status}</span>
      </div>
      ...
      <div className="nav-mobile" id="mobile-navigation">
        ...
        <div><span className={`chain-status ${tone}`.trim()}><i aria-hidden="true" />{status}</span></div>
      </div>
    </nav>
  );
}
