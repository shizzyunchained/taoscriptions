...
import { getSubnetName, type SubnetNameMap } from "@/lib/subnet-display";
...
export function MarketplaceBrowser({
  ...
  const visible = useMemo(() => {
    ...
    return listedArtifacts
      .filter((artifact) => {
        ...
        const subnetName = getSubnetName(subnetNames, artifact.netuid, artifact.subnetGeneration);
        return `${artifact.name} ${artifact.body ?? ""} ${artifact.globalNumber} ${artifact.subnetNumber} ${artifact.netuid} ${subnetName} ${artifact.purpose}`.toLowerCase().includes(normalizedQuery);
      })
      .toSorted((left, right) => {
  ...
  }, [listedArtifacts, listingByArtifact, priceBand, purpose, query, sort, subnet, subnetNames]);
        ...
        <aside className="market-filters">
          ...
          <div className="market-filter-control">
            ...
            <select id="market-subnet" value={subnet} onChange={(event) => setSubnet(event.target.value)}>
              ...
              {subnetOptions.map((netuid) => {
                const artifact = listedArtifacts.find((item) => item.netuid === netuid);
                const subnetName = artifact ? getSubnetName(subnetNames, artifact.netuid, artifact.subnetGeneration) : `Subnet ${netuid}`;
                return <option key={netuid} value={netuid}>SN{netuid} · {subnetName}</option>;
              })}
            </select>
          </div>
          <div className="market-filter-control">
        ...
        <div className="market-results">
          <header>
            ...
            <select value={sort} onChange={(event) => setSort(event.target.value as SortOrder)} aria-label="Sort marketplace items">
              ...
              <option value="burn-high">Token burn: high to low</option>
              <option value="burn-low">Token burn: low to high</option>
                  ...
                  <article className="market-card" key={artifact.artifactId}>
                    ...
                    <div className="market-card-body">
                      ...
                      <dl>
                        ...
                        <div className="subnet-burn-stat">
                          <dt>Subnet token burned</dt>
                          <dd><strong>{formatRao(artifact.alphaBurnedRao)} SN{artifact.netuid} BURNED</strong><small>{subnetName}</small></dd>
                        </div>
                      </dl>
